...
"""
Moto stores the mocked AWS infrastructure state in memory, but one "gotcha" I believe I found 
was that using seperating the modification of the mocked infrastructure state into different functions, 
meant that one service did not have the accurate state of the other (I don't think this was a matter of 
invocation order), so, still not entirely sure why this is, 
but I believe that when starting the Moto server via the CLI (as you'd do if you wanna say 
use Moto with another language), functions seem to have the updated state as expected 
"""
moto_server_port = 5001 # moto server default port is 5000
server = ThreadedMotoServer(port=moto_server_port)
server.start()
#client = boto3.client("service", endpoint_url="http://localhost:5000")
server_client = boto3.client("s3", endpoint_url="http://127.0.0.1:"+ str (moto_server_port))
#server_client = boto3.client("s3")
iam_client = boto3.client('iam', endpoint_url="http://127.0.0.1:"+ str (moto_server_port))
lambda_client = boto3.client('lambda', endpoint_url="http://127.0.0.1:"+ str (moto_server_port))
#@mock_aws
#def create_bucket():
@mock_aws
def create_bucket():
